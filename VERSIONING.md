...
Patch numbers advance by exactly one per release: `0.6.5` → `0.6.6` →
`0.6.7`. Never skip or jump patch numbers (`0.6.5` → `0.6.7` is wrong),
...
gemchain guard ask-tools
gemchain update ask-tools <new-version> --dry-run
gemchain update ask-tools <new-version>
```
...
3. Build passes — `bundle exec rake build` (or `gem build ask-tools.gemspec`).
4. Changelog — `Unreleased` entries moved under the new `X.Y.Z` heading with
   the release date.
5. Version — bumped in `lib/ask/version.rb` by gemchain, not by hand.
...
| `lib/ask/version.rb` at HEAD | `X.Y.Z` |
| Git tag | `vX.Y.Z` |
...
- Bug fix only: `0.6.5` → `0.6.6`.
- Several bug fixes ship together: one release, `0.6.5` → `0.6.6` (never
  `0.6.7`).
- New backwards-compatible API: `0.6.5` → `0.7.0`.
- Breaking change while pre-1.0: `0.7.0` → `0.8.0`, with a `Changed` /
  `Removed` changelog entry calling out the break.
- First stable release: `0.9.x` → `1.0.0`.
- This gem bumped because a dependency released: patch only, e.g. `0.6.6` →
  `0.6.7`, via the gemchain cascade — never hand-published.
